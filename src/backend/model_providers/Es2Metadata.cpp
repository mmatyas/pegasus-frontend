                                 ...
                                 QHash<QString, Types::Game*>& game_by_path)
{
    ...
    convertToAbsolutePath(xml_props[PATH_TAG], collection.sourceDirs().first());
                                ...
                                const QHash<QString, QString>& xml_props)
{
    ...
    const QString rom_dir_prefix = collection.sourceDirs().first() % '/';
